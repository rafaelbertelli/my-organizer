...
import LinkSingle from "./link-single";

export default function Layout({ children }: { children: ReactNode }) {
  const [ddlVisibility, setDdlVisibility] = useState<boolean>(false);

  function handleDropdown(element: string) {
    setDdlVisibility(!ddlVisibility);
  }
      ...
      >
        <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
          <ul className="space-y-2 font-medium">
            {ROUTES_PRIVATE.map((r) => (
              <LinkSingle key={r.id} {...r} />
            ))}
            <li>
              <a
                href="#"
            ...
            <li>
              <button
                ...
                onClick={() => handleDropdown("ecommerce")}
              >
                <svg
                  aria-hidden="true"
