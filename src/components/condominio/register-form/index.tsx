...
export default function RegisterForm(props: Props) {
  ...
  const onSubmit = (data: FormProps) => {
    const tenant = new Tenant({ name: data.tenant.name, apartment: data.tenant.apartment })
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <TextInput
            id="name"
            placeholder="João & Raquel"
            autoComplete='false'
            defaultValue="" {...register("tenant.name")}
          />
